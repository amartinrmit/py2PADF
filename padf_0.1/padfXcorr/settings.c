...
void writeConfig(settings * s){
  ...
  */
  ...
  if (outfile != NULL) {  
    ...
    fprintf(outfile,"input diffraction pattern 1 = %s\n", s->input_dp_name);
...
void parseLine(char * line, settings * s){
  ...
  if (strcmp(label,"wavelength") == 0) {
    ...
    fp = fopen( pa,   "r");
    if (fp == NULL){
      printf("diffraction pattern file 1 could not be opened - check the directory/name given in the config file\n");
      printf("%s\n", s->input_dp_name);
    } else {
      fclose( fp );
    }
