...
void allocate_blarr_t( blarr_t * blarr, int nl, int nq ){
...
void sphB_r_to_q_matrix( int nr, double rmax, int nmax, 
  ...
  for(i=0;i<nr;i++){
    ...
    for(j=0;j<nmax;j++){
      ...
      if ( (i==nr-3)&&(j<4) ){
	printf("DEBUG sphB : %d %g %d %g %g %g\n", i*nmax+j, gsl_sf_bessel_jl ( l, arg ), l, arg, qln,
	       r/rmax );
      }
    }
  }
...
void Blqq_calc_fast( correlation_t * corr, correlation_t * corrsig, blarr_t * bla, 
  ...
  blarr_t bltemp;
  allocate_blarr_t( &bltemp, bla->nl, bla->l[0].nq );
  for(i=0;i<bla->nl;i++){
...
void padf_calc( correlation_t * corr, correlation_t * corrsig, correlation_t * padf, 
		...
		double wl, int nl, int blflag, double blfilter,
  ...
  allocate_blarr_t( &blar, nl, padf->nq );
