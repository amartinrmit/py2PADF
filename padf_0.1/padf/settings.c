...
void writeConfig(settings * s){
  ...
  if (outfile != NULL) {  
    ...
    fprintf(outfile,"nl - maximum l value            = %d\n", s->nl );
    fprintf(outfile,"nlmin - minimum l value         = %d\n", s->nlmin );
...
void parseLine(char * line, settings * s){
  ...
  else if (strcmp(label,"output_padf") == 0) {
    s->output_padf = atoi(value); 
  }
