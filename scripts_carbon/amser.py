...
class serdata:
  ...
  def read_image_and_append_to_list( self, fc, cp, datoffarr, imgnum ):
...
class calparams:
	...
	print 'Image is '+str(self.ArraysizeX)+' by '+str(self.ArraysizeY)+' pixels'
...
#    tmp = fc[2:0:-1]
    ...
    offset = 30
    offset = arrayDimsList[0].evaluate_array(  fc, offset, verbose=1 )
    if sh.NumDim > 1:
        print 'Evaluating Dimension Array 1-------------------------------'
        arrayDimsList.append( arrayDims())
        offset = arrayDimsList[1].evaluate_array( fc, offset, verbose=1 )
    ...
    if sh.DataDim == 1:
        print "1D arrays not supported by the python .ser reader - quitting"
        sys.exit()
	...
	offset=datoffarr[0]
        ...
        cp = calparams()
        offset = cp.read_cal_params( fc, offset, 1 )
