...
outtag = "C9_vs_unactivated_291019"
...
plt.figure()
plt.imshow( disp[x0:pix,:thpix], extent=[0,thlim,minres,lim], origin='lower', aspect=asp, interpolation='lanczos' )
...
for i in np.arange(len(atxt)):
     plt.annotate(atxt[i], xy=(xpnt[i], ypnt[i]), xytext=(xpnt[i]+dltx,ypnt[i]+dlty), xycoords='data',fontsize=afs, color=clr[i])
     plt.scatter(xpnt[i], ypnt[i], s=14, c=clr[i], marker=mkr[i]) #,edgecolors='b')
...
for i in np.arange(len(atxt)):
     plt.annotate(atxt[i], xy=(xpnt[i], ypnt[i]), xytext=(xpnt[i]+dltx,ypnt[i]+dlty), xycoords='data',fontsize=afs, color=clr[i])
     plt.scatter(xpnt[i], ypnt[i], s=10, c=clr[i], marker=mkr[i]) #,edgecolors='b')
     plt.scatter(180.0-xpnt[i], ypnt[i], s=14, c=clrw[i], marker=mkr[i]) #,edgecolors='b')
plt.xlim([0,180])
plt.ylim([1,4.5])
...
for i in np.arange( len(pr) ):
 ...
 #    pb, = plt.plot( th, lines3[i] )
   ...
   #  plt.legend( [p,pa, pb], ['Unactivated FEM 900 frames', 'C9 FEM 900 frames', 'C5 FEM 225 frames' ])
     plt.legend( [p,pa], ['Unactivated', 'C9' ], fontsize=fs)
